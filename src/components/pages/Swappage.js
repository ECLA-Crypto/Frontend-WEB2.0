...
import Staking from "../static/Staking"
const Homepage = () => {
      ...
      <div className={`main_bg w-11/12 mx-auto pt-10 ${connectPage==='transfers'|| connectPage==='liquidity' || connectPage==='balance'?'h-screen overflow-y-auto' : connectPage==='staking' ? 'xl:h-screen xl:overflow-y-hidden' : ''}`}>
        ...
        <div className={` ${connectPage!=='wallet'&&connectPage!=='liquidity'?'inner_bg p-5 md:w-11/12 w-full': 'p-0 w-full'} mx-auto lg:w-10/12  mt-10 flex flex-col justify-between rounded`}>
          <div className="flex lg:flex-row flex-col justify-between rounded">
            <div className={`lg:w-4/6 w-full md:p-5 ${connectPage==='staking' || connectPage==='wallet' || connectPage==='liquidity'||connectPage==='transfers' ||connectPage==='nfts'||connectPage==='balance'? 'hidden' : ''}`}>
            ...
            {connectPage==='swap'? <Swap/>: connectPage==='limit' ? <Limits/>: connectPage==='liquidity' ? <Liquidity/> : connectPage==='wallet' ? <Wallet/> : connectPage==='transfers' ? <Transfers/> : connectPage==='balance' ? <Balance/> : connectPage==='nfts' ? <NFTS/> : connectPage==='staking' ? <Staking/> : <Swap/>}
          </div>
